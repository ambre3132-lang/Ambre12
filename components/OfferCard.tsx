...
const OfferCard: React.FC<OfferCardProps> = ({ offer }) => {
  ...
  return (
    <div className="bg-gradient-to-br from-rose-50 to-amber-50 rounded-lg shadow-lg p-8 flex flex-col justify-between text-center border-2 border-transparent hover:border-brand-pink transform hover:scale-105 hover:shadow-2xl transition-all duration-300 min-h-[360px]">
      <div>
        <h3 className="text-2xl font-serif font-bold text-brand-pink mb-3 min-h-[64px] flex items-center justify-center">{offerTitle}</h3>
        <p className="text-brand-gray mb-6 min-h-[70px]">{t(offer.description as TranslationKeys)}</p>
      </div>
      <div className="mt-auto">
         <p className="text-3xl font-bold text-yellow-600 mb-6">{offer.price}</p>
         <a
            href={whatsappUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-block bg-brand-pink text-white font-bold py-2 px-8 rounded-full hover:bg-pink-700 transition duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-1"
         >
            {t('book_now')}
         </a>
      </div>
